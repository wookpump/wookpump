...
class Coin:
    ...
    def __init__(self, coinName):
        ...
        # [curr_time, last_price, buy_price, sell_price, server_time, high, low, avg, volume]
        # [curr_time, last_price, bid_price, ask_price, server_time, high, low, avg, volume]
        self.list_price = [[int(time.time()), 9999, 9999, 9999, int(time.time()), 9999, 9999, 9999, 9999],
                           [int(time.time()), 9999, 9999, 9999, int(time.time()), 9999, 9999, 9999, 9999],
                           [int(time.time()), 9999, 9999, 9999, int(time.time()), 9999, 9999, 9999, 9999],
                           [int(time.time()), 9999, 9999, 9999, int(time.time()), 9999, 9999, 9999, 9999],
                           [int(time.time()), 9999, 9999, 9999, int(time.time()), 9999, 9999, 9999, 9999]
                           ]
    ...
    def setCurrentPrice(self, last_price, buy_price, sell_price, server_time, high, low, avg, vol):
        curr_time = datetime.datetime.now()
        ...
        if self.list_price[4][1] != last_price:
            ...
            self.list_price[4] = [int(time.time()), last_price, buy_price, sell_price, server_time, high, low, avg, vol]
        ...
        if self.IsInTrading == False:
            if curr_time - prev_prev_time < IGNORE_GAP_SECONDS:
                ...
                if price_gap_rate_last > 0.01:
                    ...
                    prev3_price_gap_rate_last = (prev2_prev_price_last - prev3_prev_price_last) / prev3_prev_price_last
        ...
        while True:
            try:
                ...
                for key, value in result.items():
                    dict_ALL_COIN_DATA[key].setCurrentPrice(value['last'], value['buy'], value['sell'], value['updated'], value['high'], value['low'], value['avg'], value['vol'])
    ...
    while True:
        ...
        if during_seconds > CANCEL_TIME:
            ...
            while True:
                ...
                printt("After %d seconds Try %d / 20 to Cancel and Sell %.3f" % (CANCEL_TIME, loop_count2, LAST_SELL_PRICE_RATE))
                if(loop_count2 >= 20):
