...
BUY_COIN_UNIT = 0.0005  # Total Buy bit at least 0.0005 ex)0.1 = 0.1BIT
...
SELL_PRICE_RATE = 1.2  # Sell coin at buy price(Actual) * 1.2 ex)1.2 = 120%
CANCEL_TIME = 4 # afert CANCLE_TIME seconds, cancel all open order and sell market ex) 50 = 50 seconds
...
def do_work(pbittrex, coinName):
    while True:
        result = pbittrex.get_ticker(coinName)
        if coinName == 'BTC-AUR':
            printt(coinName + ' : ' + str(result))
...
if __name__  == "__main__":
    ...
    for coin in result['result']:
        ...
        if 'BTC-' in MarketName and coin['IsActive'] and isExcludedCoin(MarketName) is not True:
            try:
                #qresult = Queue()
                p = Process(target = do_work, args=(bittrex,MarketName,))
                p.start()
            except:
